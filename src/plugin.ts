  ...
  for (const language of args.config.languages) {
    ...
    try {
      ...
      for (const languagefile of await args.$fs.readdir(`${resourcePath.replace("/*.json", "")}`)) {
        const json: any = {};
        json[languagefile.toString().replace(".json", "")] = JSON.parse(await args.$fs.readFile(`${resourcePath.replace("/*.json", "")}/${languagefile}`, { encoding: "utf-8" }) as string)
        const flatJson = flatten(json) as Record<string, string>;
...
async function writeResources(
  ...
  //console.log(args.resources[0].body)
